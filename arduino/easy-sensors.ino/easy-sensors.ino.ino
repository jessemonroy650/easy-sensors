/*
    Date: 2016-05-15
    2016-05-22 - change to BridgeServer/BridgeClient
*/
#include <Bridge.h>
// #include <YunServer.h>
// #include <YunClient.h>
...
// This from: http://playground.arduino.cc/ComponentLib/Thermistor
...
void setup() {
    // open serial connection via USB-Serial
    ...
    server.listenOnLocalhost(); // Listens on port 5555
    server.begin();
}
...
void loop() {
    ...
    if (client) {
        SERIAL_PORT_USBVIRTUAL.println("got new client");
        // Process request
        process(client);
...
void analogCommand(BridgeClient client) {
    ...
    if (client.read() == '/') {
        ...
        // This from: http://playground.arduino.cc/ComponentLib/Thermistor
        value = analogRead(THERM_PIN) - 238;
        therm = pgm_read_word(&temps[value]);

        client.print(F("Pin A"));
        client.print(pin);
        client.print(F(" reads analog "));
        //client.println(value);
        client.println(therm);

        String key = "A";
        key += pin;
        //Bridge.put(key, String(value));
        Bridge.put(key, String(therm));
    }
}
